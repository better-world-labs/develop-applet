          ...
          <n-carousel :space-between="20" :loop="false" slides-per-view="auto" draggable>
            <n-carousel-item style="width: 40%" v-for="result in applicationStore.resultList" :key="result.id">
              <div class="result-item">
                ...
                <div class="option">
                  <div>
                    <icon-font-symbol @click="resultOption(result, 1)"
                      :name="applicationStore.resultStateList?.get(result.id) == 1 ? 'icon-icon-yidianzan' : 'icon-icon-dianzan'" />
                    <div :class="{ 'active-text': applicationStore.resultStateList?.get(result.id) == 1 }">{{
                      result.likeTimes }}</div>
                  </div>
                  <div>
                    <icon-font-symbol @click="resultOption(result, -1)"
                      :name="applicationStore.resultStateList?.get(result.id) == -1 ? 'icon-icon-yicai' : 'icon-icon-cai'" />
                    <div :class="{ 'active-text': applicationStore.resultStateList?.get(result.id) == -1 }">{{
                      result.hateTimes }}</div>
                  </div>
                </div>
              </div>
            </n-carousel-item>
...
function resultOption(item = {}, val) {
  let status = val;
  if ((applicationStore.resultStateList?.get(item.id) == 1 && status == 1) ||
