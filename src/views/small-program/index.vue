      ...
      <n-grid :x-gap="12" v-else cols="1 s:1 m:2 l:3 xl:3 2xl:4" responsive="screen">
        <n-grid-item v-for="item in applicationStore.mineAppList" :key="item.id">
          <div class="item" @click="detailApp(item)">
            ...
            <div>
              已售： <span class="active-text">{{ item.soldPoints || 0 }}</span>积分
            </div>
            <div class="options">
              <div>
                <div>被使用</div>
            ...
            <div style="text-align: right">
              ...
              <n-button class="border-bg-btn" size="large" style="margin-left: 16px;"
                @click="shareTemplate($event, item)">分享</n-button>
            </div>
          </div>
        </n-grid-item>
      </n-grid>
